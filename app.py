...
# Sidebar: 9 position inputs (0=blank, 1=x, 2=o)
st.sidebar.header("Board (0=blank, 1=x, 2=o)")
board = np.array([
    st.sidebar.selectbox(f"Pos {i+1}", [0, 1, 2], key=f"p{i}") 
    for i in range(9)
])
...
if st.button(" Predict Winner"):
    ...
    prob = models['XGBoost'].predict_proba(X_scaled)[0, 1]
    
    result = "Winner! (positive)" if pred == 1 else "No winner (negative)"
    st.success(f"**{result}** | Confidence: {prob:.1%}")
